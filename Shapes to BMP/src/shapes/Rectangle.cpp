...
void Rectangle::initialize(const string &data) {
	...
	vertices.push_back(Coordinate(width, height));
	vertices.push_back(Coordinate(width, 0));
}

// Copy-pasted from Polygon. Attempting to extend Polygon repeatedly give errors claiming that base class Polygon is not defined. This is with it included in the Rectangle header and despite it working perfectly when Rectangle is not used.
void Rectangle::draw(Picture &picture) const {
...
void Rectangle::drawShadow(Picture &picture) const {
